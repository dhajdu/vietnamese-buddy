...
export async function getSettings(db: SupabaseClient): Promise<Settings> {
  ...
  return {
    freeLessonsPerWeek: (data?.free_lessons_per_week as number) ?? 1,
...
/** Throws on any failed read, so a database hiccup never hands out free generations. */
export async function getEntitlement(
  db: SupabaseClient,
  userId: string,
  opts: { pair: PairId; timezone: string; isAdmin: boolean },
): Promise<Entitlement> {
  // The week starts at Monday 00:00 in the learner's own timezone, not UTC.
  const monday = zonedMidnightUtc(weekStart(localDate(opts.timezone)), opts.timezone)

  // Usage comes from the generation ledger, which learners cannot write, so
  // deleting or editing a lesson never gives a slot back.
  const [settings, sub, plan, week, day] = await Promise.all([
    ...
    db.from('lesson_generations').select('id', { count: 'exact', head: true })
      .eq('user_id', userId).eq('pair', opts.pair).gte('created_at', monday),
    db.from('lesson_generations').select('id', { count: 'exact', head: true })
      .eq('user_id', userId).gte('created_at', new Date(Date.now() - 86_400_000).toISOString()),
  ])
  for (const r of [sub, plan, week, day]) {
    if (r.error) throw new Error(`entitlement read failed: ${r.error.message}`)
  }
