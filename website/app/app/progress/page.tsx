  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5 px-4 pt-8 sm:px-6 sm:pt-12">
      ...
      <section className="space-y-2">
        <h2 className="eyebrow">{d.last30}</h2>
        <ol className="grid grid-cols-10 gap-1.5">
          {lastNDates(today, 30).map(dt => {
            const a = days.get(dt)
            const full = a && a.lesson_created && a.cards_reviewed >= 10
            const what = a ? [a.lesson_created && 'lesson', a.cards_reviewed && `${a.cards_reviewed} cards`, a.lesson_completed && 'completed'].filter(Boolean).join(', ') : '—'
            const cls = dt === today ? 'border-red bg-red' : full ? 'border-ink bg-ink' : a ? 'border-sand-deep bg-sand-deep' : 'border-sand bg-white'
            return <li key={dt} title={`${dt}: ${what}`} className={`aspect-square rounded-md border ${cls}`}><span className="sr-only">{dt}: {what}</span></li>
          })}
        </ol>
        <p className="meta">{d.calendarKey}</p>
      </section>

      <StatTiles cols="grid-cols-2 sm:grid-cols-4" stats={[
        { label: d.lessonsDone, value: completed.count ?? 0 },
        { label: d.phrases, value: phrases },
        { label: d.wordsMet, value: words },
        { label: d.wordsKnown, value: vocabKnown.count ?? 0 },
      ]} />

      <div className="card border-l-[5px] border-l-red px-4 py-3">
        <p className="eyebrow mb-1">{d.nextMilestone}</p>
