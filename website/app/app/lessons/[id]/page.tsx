...
// The tab title and the page share one lesson read per request.
const loadLesson = cache(async (id: string) => {
  const user = await requireAuth()
  return getLesson(await createClient(), user.id, id)
})

export async function generateMetadata({ params }: PageProps<'/app/lessons/[id]'>): Promise<Metadata> {
  const row = await loadLesson((await params).id)
  return { title: row?.lesson_json.title ?? 'Lesson' }
}
...
export default async function LessonPage({ params }: PageProps<'/app/lessons/[id]'>) {
  ...
  const [row, { isAdmin, timezone }] = await Promise.all([loadLesson(id), getCurrentProfile(user.id)])
  if (!row) notFound()
  ...
  // The page speaks the learner's language; only the language being learned is marked otherwise.
  ...
  return (
    <article lang={pair.uiLocale}>
      <header className="bg-ink-warm px-6 pb-7 pt-6 text-sand sm:pt-10">
        <div className="mx-auto max-w-3xl space-y-3">
          <nav aria-label={d.navLessons} className="text-xs font-semibold text-sand-70">
            <Link href="/app/lessons" className="inline-flex min-h-11 items-center hover:text-sand sm:min-h-0">← {d.navLessons}</Link>
            {parent && <> / <Link href={`/app/lessons/${parent.id}`} lang={targetLang} className="inline-flex min-h-11 items-center hover:text-sand sm:min-h-0">{parent.title as string}</Link></>}
          </nav>
          <h1 lang={targetLang} className="flex items-center gap-3 font-vn text-[26px] font-extrabold leading-[1.05] tracking-[-0.03em] text-sand sm:text-[34px]">
            {lesson.title}
          </h1>
      ...
      <div className="mx-auto max-w-3xl space-y-4 px-4 pt-4 sm:px-6 sm:pt-6">
            ...
            {lesson.phrases.map((p, i) => (
              <li key={i} className="py-3 sm:grid sm:grid-cols-[1.1fr_0.9fr] sm:gap-6">
                <div>
                  ...
                  <p lang={targetLang} className="target text-[22px] leading-[1.15]">{p[pair.targetField]}</p>
        ...
        <section className="room px-4 py-4 sm:px-5">
          ...
          <p lang={targetLang} className="mb-2 inline-block rounded-lg border-l-[3px] border-red-bright bg-on-dark-soft px-2.5 py-1.5 font-vn text-[15px] font-bold text-sand">{lesson.grammar.pattern}</p>
            ...
            {lesson.grammar.examples.map((e, i) => (
              <li key={i} className="py-2.5">
                <div className="flex items-start gap-2">
                  <p lang={targetLang} className="font-vn text-[18px] font-extrabold leading-snug tracking-[-0.02em] text-sand">{e[pair.targetField]}</p>
            ...
            {lesson.vocabulary.map((v, i) => (
              <li key={i} className="flex items-center justify-between gap-3 border-b border-sand/70 py-1.5">
                <span className="flex items-center gap-2">
                  ...
                  <span lang={targetLang} className="font-vn text-[16px] font-bold text-ink">{v[pair.targetField]}</span>
                </span>
