...
export default async function VocabularyPage({ searchParams }: PageProps<'/vocabulary'>) {
  ...
  const [{ data: rows }, { data: counts }] = await Promise.all([query, db.from('vocabulary').select('status').eq('user_id', user.id)])
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5 px-4 pt-8 sm:px-6 sm:pt-12">
      <header>
        ...
        <h1 className="t-title text-3xl sm:text-4xl"><span className="tabular font-vn font-extrabold">{tally.total}</span> words learned</h1>
        <p className="tabular meta mt-1 text-sm">{tally.known} known · {tally.learning} learning · {tally.new} new</p>
      </header>

      <form className="flex gap-2" method="get">
        <input type="search" name="q" defaultValue={q} placeholder="Search Vietnamese or English" className="input flex-1" />
      ...
      {!rows?.length ? (
        <div className="rounded-card border border-dashed border-sand px-4 py-8 text-center text-sm text-stone">Nothing here yet.</div>
      ) : (
            ...
            return (
              <li key={r.id} className="card px-4 py-2.5">
                <div className="flex items-center justify-between gap-3">
                  <span className="flex items-center gap-2"><Speak text={r.vietnamese} size="sm" /><span className="font-vn text-[16px] font-bold text-ink">{r.vietnamese}</span></span>
                  <StatusToggle id={r.id} status={r.status} />
                </div>
                <p className="t-gloss text-[14px]">{r.english}</p>
                {intro && <p className="meta"><Link href={`/lessons/${intro.id}`} className="hover:text-ink">{intro.title}</Link> · {formatDate(intro.created_at, timezone)}</p>}
              </li>
            )
          })}
        </ul>
