...
export default async function AdminUserPage({ params }: PageProps<'/admin/users/[id]'>) {
  ...
  return (
    <div className="space-y-6">
      <nav className="text-xs font-semibold text-body"><Link href="/admin" className="hover:text-ink">← Users</Link></nav>
            ...
            <tbody>
              {lessons.map(l => (
                <tr key={l.id} className="border-t border-sand">
                  ...
                  <td className="p-3 text-body">{l.pair}</td>
                  <td className="p-3 text-body">{l.source}</td>
                  <td className="p-3 text-body">{l.completed_at ? '✓' : '—'}</td>
                  <td className="p-3 text-body">{l.created_at.slice(0, 10)}</td>
                </tr>
              ))}
              {!lessons.length && <tr><td colSpan={5} className="p-8 text-center text-body">No lessons yet.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
